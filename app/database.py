...
load_dotenv()# reads .env and injects values into os.environ
...
#The general principle, worth keeping beyond just this case: anything that needs to be shared and consistent across multiple files — Base, engine, SessionLocal — should be defined in exactly one place and imported everywhere else it's needed, never redefined.
# This is the same reasoning, generalized, behind why we were careful earlier about SessionLocal living in database.py rather than being recreated inside every route file.